{{-- タグ変更画面 --}}
@extends('layouts.app')
@section('title', __('messages.edit_value', ['value' => __('messages.tag')]))
@section('scripts')
    <script src="{{ asset('js/tag.js') }}" defer></script>
@endsection
@section('content')
    <div class="page-header">
        <h3>{{ __('messages.edit_value', ['value' => __('messages.tag')]) }}</h3>
    </div>
    <div class="py-3">
    ...
    <form action="" method="post" class="form-horizontal" id="tag-page">
        @csrf
        <div class="form-group row @if($errors->has('name')) has-error @endif">
            <label for="name" class="col-md-3 col-form-label text-md-right">{{ __('messages.tag_name') }}</label>
            <div class="col-md-6">
                <input type="text" class="form-control" id="name" name="name"
                       value="{{ old('name', $tag->name) }}" ref="text" data-value="{{ old('name', $tag->name) }}" v-on:input="changeTagText">
        ...
        <div class="form-group row @if($errors->has('color')) has-error @endif">
            ...
            <div class="col-md-2">
                <input type="color" class="form-control" id="color" name="color"
                       value="{{ old('color', $tag->color) }}" ref="color" data-value="{{ old('color', $tag->color) }}" v-on:change="changeTagColor">
        ...
        <div class="form-group row @if($errors->has('background_color')) has-error @endif">
            ...
            <div class="col-md-2">
                <input type="color" class="form-control" id="background_color" name="background_color"
                       value="{{ old('background_color', $tag->background_color) }}" ref="bgColor" data-value="{{ old('background_color', $tag->background_color) }}" v-on:change="changeTagBackgroundColor">
